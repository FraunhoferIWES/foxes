...
def clean():
    if os.path.isfile("clean.sh"):
        res = subprocess.run(["bash", "clean.sh"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ret = res.returncode
        if ret:
...
if __name__ == '__main__':
    ...
    parser.add_argument("-i", "--include", help="Include keywords for directory names, or None for all", nargs='+', type=str, default=None)
    parser.add_argument("-e", "--exclude", help="Exclude keywords for directory names", nargs='+', type=str, default=[])
    parser.add_argument("-o", "--incopt", help="Flag for inclusion of optimization cases", action='store_true', default=False)
    parser.add_argument("-O", "--forceopt", help="Flag for inclusion of optimization cases only", action='store_true', default=False)
    parser.add_argument("-d", "--dry", help="Dry run, only print directories", action='store_true', default=False)
    parser.add_argument("-D", "--Dry", help="Dry run, only print names", action='store_true', default=False)
    parser.add_argument("-s", "--step", help="Set the start step, default=0", type=int, default=0)
