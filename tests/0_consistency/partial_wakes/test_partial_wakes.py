...
@pytest.fixture(scope="module")
def gaussian_lookup_axiwake_dataset():
    r_axis, s_axis = foxes.utils.create_lookup_axes(
        r_over_sigma_max=28.0,
        n_r=121,
        sigma_over_d_min=0.02,
    )
    return foxes.utils.build_lookup_dataset(
        r_over_sigma=r_axis,
        sigma_over_d=s_axis,
        n_rho=256,
...
def test_gaussian_lookup_close_to_axiwake9_bastankhah2014(
    xy_step,
    gaussian_lookup_axiwake_dataset,
):
    thisdir = Path(inspect.getabsfile(inspect.currentframe())).parent
    tfile = thisdir / "NREL-5MW-D126-H90.csv"
    ...
    with foxes.Engine.new("threads", chunk_size_states=1):
        ...
        mbook_lookup.partial_wakes["gaussian_lookup_test"] = (
            foxes.models.partial_wakes.PartialGaussianLookup(
                lookup_data=gaussian_lookup_axiwake_dataset
            )
        )
...
def test_gaussian_lookup_matches_grid100_for_x_y_range(
    x_over_d,
    y_over_d,
    ...
    with foxes.Engine.new("threads", chunk_size_states=1):
        mbook_grid100 = foxes.models.ModelBook()
        rews_grid100 = _calc_downstream_rews(
            "rotor_points",
            mbook_grid100,
            rotor_model="grid100",
        )
        ...
        mbook_analytical = foxes.models.ModelBook()
        rews_analytical = _calc_downstream_rews("gaussian", mbook_analytical)

    err = abs(rews_lookup - rews_grid100)
    assert err < 0.12, (
        f"lookup/grid100 mismatch for x/D={x_over_d}, y/D={y_over_d}, "
        f"xy_step={xy_step}: error={err:.4f}"
    )
    np.testing.assert_allclose(rews_analytical, rews_grid100, rtol=0.0, atol=0.02)
    np.testing.assert_allclose(rews_analytical, rews_lookup, rtol=0.0, atol=0.002)
