...
def test_turbine_point_cloud_interpolate_falls_back_to_nearest_on_qhull_error():
    ...
    idims = [FC.TURBINE]
    icrds = [np.array([[[0.0, 0.0, 90.0], [300.0, 0.0, 90.0]]], dtype=float)]
    pts = np.array([[[0.0, 0.0, 90.0], [100.0, 0.0, 90.0]]], dtype=float)
    d = np.array([[[8.0, 270.0], [9.0, 271.0]]], dtype=float)
    vrs = [FV.WS, FV.WD]
    times = np.array([0], dtype=np.int32)

    out = states.interpolate_data(idims, icrds, d, pts, vrs, times)
