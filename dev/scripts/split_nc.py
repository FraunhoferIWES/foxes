...
for si, s in enumerate(slices):
    ...
    var_height = ncfile.createVariable(
        "height", np.float32, ("height",), zlib=True, least_significant_digit=1
    )
    var_height.units = "m"
    var_height[:] = sdata["height"].values

    var_UTMY = ncfile.createVariable(
        "UTMY", np.float32, ("UTMY",), zlib=True, least_significant_digit=2
    )
    var_UTMY.units = "m"
    var_UTMY[:] = sdata["UTMY"].values - sdata["UTMY"].mean().values

    var_UTMX = ncfile.createVariable(
        "UTMX", np.float32, ("UTMX",), zlib=True, least_significant_digit=2
    )
    var_UTMX.units = "m"
    var_UTMX[:] = sdata["UTMX"].values - sdata["UTMX"].mean().values

    var_WS = ncfile.createVariable(
        "WS",
        np.float32,
