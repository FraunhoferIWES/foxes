...
if __name__ == "__main__":
    ...
    if ofile.is_file() and not args.force:
        ...
        tresults = pd.DataFrame(index=minds, columns=['n_states', 'chunksize', 'n_cpus', 'n_threads', 'time'])
    
    idx = pd.IndexSlice
    tresults.loc[idx[l, n_t], 'n_states']  = n_s
    tresults.loc[idx[l, n_t], 'chunksize'] = c
    tresults.loc[idx[l, n_t], 'n_cpus'] = args.n_cpus
    tresults.loc[idx[l, n_t], 'n_threads'] = args.threads_per_cpu
