...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--ofile", help="The output graphics file", default="results.png"
    )
    ...
    data = data.loc[data["chunksize"] == 10000]
    print(data)
    ...
    for s, g in data.groupby("scheduler"):
        ...
        g.sort_values("n_turbines", axis=0, inplace=True)

        ax.plot(g["n_turbines"], g["time"], label=s)

        if t is None:
            t = f"{g['n_states'].iloc[0]} states, 1 row, chunksize {g['chunksize'].iloc[0]}"
