...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-w", "--wake", help="The wind wake model", default="Bastankhah_rotor"
    )
    parser.add_argument(
        "-s",
        "--superp",
        help="The wind wake superposition model",
        default="wind_linear",
    )
    ...
    if ofile.is_file() and not args.force:
        ...
        minds = pd.MultiIndex.from_product(
            [[l], [c]], names=["scheduler", "n_turbines"]
        )
        tresults = pd.DataFrame(
            index=minds, columns=["n_states", "chunksize", "n_cpus", "time"]
        )
