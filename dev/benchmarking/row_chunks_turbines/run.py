...
if __name__ == "__main__":
    ...
    parser.add_argument("-w", "--wake", help="The wind wake model", default="Bastankhah_linear")
    ...
    if ofile.is_file() and not args.force:
        tresults = pd.read_csv(ofile).set_index(["scheduler", "n_turbines", 'chunksize'])
    else:
        minds = pd.MultiIndex.from_product([[l], [n_t], [c]], names=['scheduler', 'n_turbines', 'chunksize'])
        tresults = pd.DataFrame(index=minds, columns=['n_states', 'n_cpus', 'n_threads', 'time'])
