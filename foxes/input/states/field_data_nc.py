    ...
    def model_input_data(self, algo):
        ...
        with xr.open_mfdataset(self.file_pattern, parallel=True, 
            ...
            if self.pre_load:
                ...
                idata["coords"][self.H]    = ds[self.h_coord].values
                idata["coords"][self.Y]    = ds[self.y_coord].values
                idata["coords"][self.X]    = ds[self.x_coord].values
                idata["coords"][self.VARS] = list(self._dkys.keys())

                coos = (FV.STATE, self.H, self.Y, self.X, self.VARS)
                data = self._get_data(ds)
                idata["data_vars"][self.DATA] = (coos, data)
