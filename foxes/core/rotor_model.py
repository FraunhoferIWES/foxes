    ...
    def eval_rpoint_results(
        ...
        if (
            ...
            for v in self.calc_vars:
                ...
                if v == FV.REWS:
                    rews = np.maximum(np.einsum("stp,p->st", wsp, weights), 0.)
                    ...
                    if uvp.shape[2] > 1:
                        rews2 = np.sqrt(np.maximum(
                            np.einsum("stp,p->st", np.sign(wsp) * wsp**2, weights), 0.))
                    else:
                    ...
                    if uvp.shape[2] > 1:
                        rews3 = np.maximum(
                            np.einsum("stp,p->st", wsp**3, weights), 
                            0.) ** (1.0 / 3.0)
                    else:
