...
from .multiprocess import MultiprocessEngine, _run_as_proc
        ...
        **calc_pars,
        ...
        for chunki_states in range(n_chunks_states):
            ...
            for chunki_points in range(n_chunks_targets):
                ...
                jobs[(chunki_states, chunki_points)] = self._pool.submit(
                    _run_as_proc,
                    algo, 
                    model, 
                    data, 
        ...
        for chunki_states in range(n_chunks_states):
            for chunki_points in range(n_chunks_targets):
                ...
                results[key] = jobs.pop((chunki_states, chunki_points)).result()
                if pbar is not None:
                    pbar.update()
        if pbar is not None:
            pbar.close()
