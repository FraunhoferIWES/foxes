...
def calc_era5_density(era5, z, var2ncvar={}):
    ...
    mc = import_module(
        "metpy.calc",
        pip_hint="pip install metpy",
        conda_hint="conda install -c conda-forge metpy",
    )
    mu = import_module(
        "metpy.units",
        pip_hint="pip install metpy",
    ...
    pz = (
        p0
        / mc.height_to_pressure_std(0 * mu.units.meter)
        * mc.height_to_pressure_std(z)
    )
    ...
    m = mc.mixing_ratio_from_relative_humidity(
        pressure=pz, temperature=tz, relative_humidity=rh
    )
