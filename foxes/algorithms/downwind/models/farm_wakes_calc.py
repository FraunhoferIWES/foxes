...
class FarmWakesCalculation(FarmDataModel):
    ...
    def initialize(self, algo, farm_data):
        ...
        if not self.pwakes.initialized:
            self.pwakes.initialize(algo, farm_data)
    ...
    def calculate(self, algo, fdata):
        ...
        wdeltas = self.pwakes.new_wake_deltas(algo, fdata)
        ...
        for oi in range(n_order):
            ...
            if oi > 0:
                ...
                self.pwakes.evaluate_results(algo, fdata, wdeltas, states_turbine=o)

                trbs = np.zeros((n_states, algo.n_turbines), dtype=bool)
                np.put_along_axis(trbs, o[:, None], True, axis=1)

                mres = algo.farm_controller.calculate(algo, fdata, st_sel=trbs)
                fdata.update(mres)

            if oi < n_order - 1:
                self.pwakes.contribute_to_wake_deltas(algo, fdata, o, wdeltas)

        return {v: d for v, d in fdata.items() if v in self.output_farm_vars(algo)}
